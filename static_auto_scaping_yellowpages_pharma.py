...
google: list pharmaceutical companies in metro manila
...
class YellowPagesPhScraper:
    def __init__(self):
        ...
        self.search_what_trimmed = "pharmaceutical"
        self.search_what = "Pharmaceutical"
        ...
        self.image_file = os.path.join(self.script_dir, "eeak_logo600x200.png")  #  bg="#225479"
        ...
        for key in self.dict_business_info.keys():
            ...
            row += 1
        self.button_save = tk.Button(self.frame_upper, text=f"Save Details to {self.save_filepath}", command=self.save_file, font=self.font_bold)
        self.button_save.grid(columnspan=3, row=row, column=0, ipadx=15, ipady=5) 
        row += 1
        for i in range(2): # Additional invisible rows
            tk.Label(self.frame_upper, bg="#225479").grid(columnspan=3, row=row, column=0,  pady=5)
            row += i+1
            
        # Contest Display ==================== Text field version ===============
        # self.tview_df_contents = tk.Text(self.frame_upper, font=self.font_default)
        # self.tview_df_contents.grid(rowspan=self.MAXROWS-5, columnspan=self.MAXCOLUMNS-2, row=self.BELOW_WEPAGE, column=3, sticky="nsw")
        ...
        self.frame_lower.grid(columnspan=self.MAXCOLUMNS-2, row=row+2, column=2, sticky="nswe", ipadx=10)
    ...
    def start_auto_scraping(self):
        ...
        for pagenum in range(1, 5):
            ...
            for count, link in enumerate(all_h2_elements):
                ...
                not_pharma_count = sum(1 for each in self.tuple_not_pharma if each in h2_link_lowercase_text)
        ...
        self.button_save['text'] = f"Save Details to {self.save_filepath}"
    ...
    def udpate_tview_df_contents(self, data_frame):
        ...
        for column in self.tview_df_contents["columns"]:
