...
class YellowPagesPhScraper:
    def __init__(self):
        ...
        self.frame_upper.grid(
            rowspan=self.MAXROWS,
            columnspan=self.MAXCOLUMNS,
            row=0,
            column=0,
            padx=5,
            pady=5,
        ...
        self.label_imagelogo = tk.Label(
            self.frame_upper, image=self.imagelogo, bg="#225479"
        )
        self.label_imagelogo.grid(rowspan=4, columnspan=3, row=0, column=0, sticky="nw")
        self.BELOW_LOGO = 4

        self.label_webpage = tk.Label(
            self.frame_upper,
        ...
        self.label_webpage.grid(
            columnspan=5, row=self.BELOW_LOGO, sticky="wes", column=0, ipady=5
        )

        label_search_what = tk.Label(
            self.frame_upper,
            text="Search:",
        ...
        self.entry_search_what = tk.Entry(
            self.frame_upper, width=40, font=self.font_bold, fg="grey"
        )
        ...
        label_search_location = tk.Label(
            self.frame_upper,
            text="Location:",
            font=self.font_large,
            fg="#c5e4fb",
        ...
        self.entry_search_location = ttk.Combobox(
            self.frame_upper,
            width=40,
            font=self.font_bold,
            foreground="grey",
        ...
        self.entry_search_location.grid(
            columnspan=2, row=2, column=4, sticky="w", ipady=10
        )
        ...
        self.button_go.grid(
            row=self.BELOW_LOGO, column=5, columnspan=2, sticky="w", ipadx=20
        )
        ...
        self.button_stop.grid(
            row=self.BELOW_LOGO, column=5, columnspan=2, sticky="e", ipadx=20
        )
        ...
        for key in self.dict_business_info.keys():
            ...
            self.dict_entries_business[key] = tk.Entry(
                self.frame_upper, width=30, font=self.font_default
            )
            self.dict_entries_business[key].grid(
                columnspan=2, row=row, column=1, sticky="w", ipady=5
            )
        ...
        self.frame_ofTview = tk.LabelFrame(
            self.frame_upper, text="Logs/DataFrames", fg="lightgrey", bg="#225479"
        )
        self.frame_ofTview.grid(
            row=self.BELOW_WEPAGE,
            column=3,
            rowspan=row - 4,
            columnspan=4,
        ...
        self.tview_df_contents.place(
            relwidth=1, relheight=1
        )  # Relative to the size of its frame_ofTview container
        tview_xscroll = tk.Scrollbar(
            self.frame_ofTview,
            orient="horizontal",
            command=self.tview_df_contents.xview,
        )  # command=update xaxis view of frame_ofTview
        tview_xscroll.pack(
            side="bottom", fill="x"
        )  # make the scrollbar fill the x axis of the Treeview widget
        tview_yscroll = tk.Scrollbar(
            self.frame_ofTview, orient="vertical", command=self.tview_df_contents.yview
        )  # command=update yaxis view of frame_ofTview
        tview_yscroll.pack(
            side="right", fill="y"
        )  # make the scrollbar fill the y axis of the Treeview widget
        self.tview_df_contents.configure(
            xscrollcommand=tview_xscroll.set, yscrollcommand=tview_yscroll.set
        )  # assign the scrollbars to frame_ofTview

        # FRAME 2 of 2: for Button: Browse File and Entry: Filepath
        self.frame_lower = tk.LabelFrame(
            self.window, text="Open and Browse DataFrames", fg="lightgrey", bg="#225479"
        )
        self.frame_lower.grid(
            columnspan=self.MAXCOLUMNS - 2,
            row=row + 4,
            column=1,
            sticky="nswe",
            ipadx=10,
        ...
        button_browse_file = tk.Button(
            self.frame_lower,
            text="Open/Browse Excel File:",
            command=self.select_excel_file,
            font=self.font_bold,
        )
        button_browse_file.grid(
            columnspan=2, row=self.MAXROWS, column=0, sticky="e", ipadx=5, padx=5
        )

        self.entry_filepath = tk.Entry(
            self.frame_lower, width=50, font=self.font_default
        )
        self.entry_filepath.grid(
            row=self.MAXROWS, column=4, columnspan=5, padx=5, pady=5, ipady=5
        )
    ...
    def capture_search_entries(self):
        while True:
            ...
            if self.entry_search_location.get() == "":
                ...
                self.entry_search_location.grid()
                delay_seconds += 2
            self.search_what_trimmed = "".join(
                byte if byte.isalpha() else "-" for byte in self.entry_search_what.get()
            )
            self.search_what_trimmed = self.search_what_trimmed.replace("--", "-")
            search_location = "".join(
                byte if byte.isalpha() else "-"
                for byte in self.entry_search_location.get()
            )
    ...
    def start_thread_capture_search_entries(self):  # Call this function inside main()
        searchchanges_thread = threading.Thread(
            target=self.capture_search_entries
        )  # From threading module
        searchchanges_thread.daemcon = (
    ...
    def start_auto_scraping(self):
        ...
        self.eeak_logs.append(self.search_what_trimmed + "\n")
        search_location = "".join(
            byte if byte.isalpha() else "-" for byte in self.entry_search_location.get()
        )
        ...
        for pagenum in range(1, max_pages + 1):
            ...
            all_h2_elements = [
                h2_element.parent.parent.parent for h2_element in all_h2_contents
            ]
            ...
            for count, link in enumerate(all_h2_elements):
                ...
                not_pharma_count = sum(
                    1
                    for each in self.tuple_not_hospital
                    if each in h2_link_lowercase_text
                )
                if not_pharma_count == 0:
                    if "hospital" in h2_link_lowercase_text:
                        self.eeak_logs.append(
                            "hospital" + h2_link_lowercase_text + "\n"
                        )
    ...
    def scrape_webpage(self, url=""):
        ...
        try:
            self.dict_business_info["Trade Name"] = soup.find(
                "h1", class_="h1-tradename"
            ).text
        except AttributeError:
            try:
                self.dict_business_info["Trade Name"] = soup.find(
                    "h1", class_="h1-single-businessname"
                ).text
            except AttributeError:
                self.dict_business_info["Trade Name"] = ""
        try:
            self.dict_business_info["Short Description"] = soup.find(
                "h2", class_="h2-businessname"
            ).text
        except AttributeError:
        ...
        try:
            self.dict_business_info["Address"] = soup.find(
                "a", class_="biz-link yp-click"
            ).text
        except AttributeError:
            self.dict_business_info["Address"] = ""

        try:
            self.dict_business_info["Contact Number"] = soup.find(
                "span", class_="phn-txt"
            ).text
        except AttributeError:
        ...
        try:
            self.dict_business_info["Email Address"] = soup.find(
                "a", class_="email-link"
            ).text
        except AttributeError:
        ...
        try:
            self.dict_business_info["Facebook URL"] = (
                "https://facebook.com/"
                + soup.find(
                    "a", class_="biz-link d-block ellipsis yp-click social-media-link"
                ).text
            )
            self.dict_business_info["Facebook URL"] = self.dict_business_info[
                "Facebook URL"
            ].replace(" ", "")
        except AttributeError:
            self.dict_business_info["Facebook URL"] = ""
        ...
        try:
            self.dict_business_info["Website"] = soup.find(
                "a", class_="biz-link d-block ellipsis yp-click"
            ).text
        except AttributeError:
            try:
                self.dict_business_info["Website"] = soup.find(
                    "a", class_="website-link"
                ).text
                if self.dict_business_info["Website"].endswith("/"):
                    self.dict_business_info["Website"] = self.dict_business_info[
                        "Website"
                    ].replace("/", "")
            except AttributeError:
    ...
    def udpate_tview_df_contents(self, data_frame):
        ...
        for column in self.tview_df_contents["column"]:
            self.tview_df_contents.heading(
                column, text=column
            )  # Headers will be all column names

        data_frame_rows = (
            data_frame.to_numpy().tolist()
        )  # Rows from Excel will become list of NumPy Arrays, then just list
    ...
    def select_excel_file(self):
        ...
        if self.save_filepath:
            self.entry_filepath.delete(0, tk.END)
            self.entry_filepath.insert(0, self.save_filepath)
            self.df_business_infos = (
                pd.read_excel(self.save_filepath)
                if self.save_filepath.endswith("xlsx")
                else pd.read_csv(self.save_filepath)
            )
            self.df_business_infos.dropna(
                inplace=True
    ...
    def save_file(self):
        ...
        self.df_business_infos = self.df_business_infos._append(
            self.dict_business_info, ignore_index=True
        )
    ...
    def delay_by_randomseconds(self):
        ...
        for i in range(1, delaysecond):  # Applying delay in requesting webpages
            self.eeak_logs.append(
                f"Automated in {delaysecond-i} {'second' if delaysecond-i == 1 else 'seconds'}...\n"
            )
            time.sleep(1)
